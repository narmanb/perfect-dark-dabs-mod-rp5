...
idx = Path("port/src/input.c").read_text(encoding="utf-8").find(anchor)
if idx < 0:
    raise SystemExit("turn boost API anchor missing")
p = Path("port/src/input.c")
s = p.read_text(encoding="utf-8")
api = '''f32 inputControllerGetTurnBoost(s32 cidx) { return padsCfg[cidx].turnBoost; }
...
p = Path("port/include/input.h")
s = p.read_text(encoding="utf-8")
anchor = 's32 inputControllerUsesFullRightStickRange(s32 cidx);\n'
if anchor not in s:
    raise SystemExit("input header generated anchor missing")
...
'''
p.write_text(s.replace(anchor, decl, 1), encoding="utf-8")

# Advanced Stick Calibration gets one entry for a dedicated submenu so the
# already-long calibration page does not overflow or become hard to navigate.
...
if anchor not in s:
    raise SystemExit("menu declaration anchor missing")
...
'''
if old not in s:
    raise SystemExit("turn boost menu row anchor missing")
s = s.replace(old, new, 1)
anchor = 'struct menudialogdef g_ExtendedStickMenuDialog = {\n'
if anchor not in s:
    raise SystemExit("stick dialog anchor missing")
...
# Runtime camera boost. It is horizontal only and is applied after the ordinary
# controller turn calculation, which lets it raise the sustained maximum turn
# speed in both free-look and controller aiming paths. Mouse-only look does not
# trigger the ramp because the trigger uses the processed physical look stick.
p = Path("src/game/bondmove.c")
s = p.read_text(encoding="utf-8")
anchor = '\tif (movedata.cannaturalturn) {\n'
if anchor not in s:
    raise SystemExit("natural turn anchor missing")
state = '''#ifndef PLATFORM_N64
\tstatic f32 turnboostlevel[INPUT_MAX_CONTROLLERS] = { 0.f };
#endif
