...
def main() -> None:
    ...
    old = """static void gfx_opengl_update_framebuffer_parameters(int fb_id, uint32_t width, uint32_t height, uint32_t msaa_level,\n                                                     bool opengl_invert_y, bool render_target, bool has_depth_buffer,\n                                                     bool can_extract_depth) {\n    Framebuffer& fb = framebuffers[fb_id];\n"""
    new = """static void gfx_opengl_update_framebuffer_parameters(int fb_id, uint32_t width, uint32_t height, uint32_t msaa_level,\n                                                     bool opengl_invert_y, bool render_target, bool has_depth_buffer,\n                                                     bool can_extract_depth) {\n#if defined(__ANDROID__)\n    // Resizing/reconfiguring framebuffer colour textures binds them directly.\n    gfx_opengl_android_invalidate_texture_cache();\n#endif\n    Framebuffer& fb = framebuffers[fb_id];\n"""
    src = replace_once(src, old, new, "Android texture cache invalidation on framebuffer update")
